...
baseConfig = '''
{
  ...
  "save": {"results":"$(RESULT_DIR)/$(TRIALNUM)-$(JOBNUM).csv"},
  "dir": "$(DIR)"
}
...
config = config.replace('$(TRIALS_PER_JOB)',str(1))
...
config = config.replace('$(TRIALNUM)',str(oldTrialNumber))
