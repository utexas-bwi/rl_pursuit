...
void WorldBeliefMDP::addAgent(AgentType agentType, boost::shared_ptr<Agent> agent) {
  WorldMDP::addAgent(agentType,agent);
  mdp->addAgent(agentType,boost::shared_ptr<Agent>(agent->clone()));
}
